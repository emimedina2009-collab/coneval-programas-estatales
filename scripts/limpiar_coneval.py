...
def procesar_anio(anio, ruta):
    ...
    df = df[~df["entidad"].astype(str).str.startswith("NOTA")]
    ...
    n_estados = df["entidad"].nunique()
    print("  Filas: " + str(len(df)) + " | Estados: " + str(n_estados))
    return df
...
resumen = panel.groupby(["cve_ent", "entidad", "anio"]).agg(num_programas=("entidad", "count")).reset_index()

if "presupuesto_ejercido_mdp" in panel.columns:
    panel["presupuesto_ejercido_mdp"] = pd.to_numeric(panel["presupuesto_ejercido_mdp"], errors="coerce")
    presup = panel.groupby(["cve_ent", "entidad", "anio"])["presupuesto_ejercido_mdp"].sum().reset_index()
...
print("Anos: " + str(sorted(resumen["anio"].unique())))
